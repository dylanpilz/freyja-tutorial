# Step 1. Primer trimming with ivar trim
...
`ivar trim -i sample_1.bam -b MPXV_primers_v1.2.bed -p trimmed`
...
- -p trimmed tells ivar that we want to output the trimmed reads with the prefix "trimmed", so the resulting file will be called trimmed.bam

When you're done, go ahead and `cd ../Step2` for the next step!